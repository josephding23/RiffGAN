...
def get_note_arrange_list(pm, bpm=120):
...
def get_accent(pm, is_raw=False):
    accent_list = []
    note_arrange_list = get_note_arrange_list(pm)
    ...
    if is_raw:
        ...
        for i, time in enumerate(accent_list):
            if time % 4 <= 2:
                accent_list[i] = time - time % 4
            else:
                accent_list[i] = time + 1
        return accent_list
